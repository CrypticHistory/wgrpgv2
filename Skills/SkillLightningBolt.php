...
class SkillLightningBolt{
	...
	public function castedByNPC($objPlayer, $objNPC){
		...
		if($objRPGCombatHelper->calculateEvadeRoll($objNPC, $objPlayer)){
			$strReturnText = $objNPC->getNPCName() . " charges energy and shoots and shoots a bolt of electricity at " . $objPlayer->getNPCName() . ", but they dodge it successfully!";
		}
		else{
			...
			$strReturnText = $objNPC->getNPCName() . " charges energy and shoots and shoots a bolt of electricity at " . $objPlayer->getNPCName() . ". It connects, inflicting " . $intDamage . " damage.";
		}
